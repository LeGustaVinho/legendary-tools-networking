...
using UnityEngine; // Unity has an outdated version of Mono that doesn't have the NetworkInformation namespace.
...
namespace LegendaryTools.Networking
{
    ...
    public class UPnP
    {
        ...
        private void ThreadDiscover(object obj)
        {
            ...
            {
#if UNITY_EDITOR
                Debug.LogWarning("[TNet] UPnP discovery failed. TNet won't be able to open ports automatically.");
#else
        ...
        private string SendRequest(string action, string content, int timeout, int repeat)
        {
            ...
            try
            {
                for (int i = 0; i < repeat; ++i)
                {
                    ...
                    response = NetworkUtility.GetResponse(web);
